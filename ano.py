...
class Ano(Printavel):
	...
	def __init__(self, ano):
		...
		conn = conectar()
		c = conn.cursor()
		c.execute("SELECT ultimoinserido, id, concluido FROM anos WHERE ano = " + str(self.ano))
		row = c.fetchone()
		if(row != None):
			self.id = row[1]
			...
			c.execute("INSERT INTO anos(ano) VALUES (" + str(self.ano) + ")")
			self.ultimoinserido = ""
	...
	def atualizar(self, link):
		conn = conectar()
		c = conn.cursor()
		c.execute("UPDATE anos SET ultimoinserido = ? WHERE id = ?", (link, str(self.id)))
		conn.commit()
		desconectar(conn)
	...
	def concluir(self):
		conn = conectar()
		c = conn.cursor()
		c.execute("UPDATE anos SET concluido = 1 WHERE id = ?", (str(self.id)))
		conn.commit()
		desconectar(conn)
